...
const CatalogItem = ({
  ...
  const item = {id, title, price: sale > 0 ? price - (price * sale) / 100 : price, size, sale, img, inStock, count}

  const { cart, addItemToCart } = useCart()


  const funcForCheckWorking = (item) => {
    addItemToCart(item)
    setIsAddedToCart(isAddedToCart => !isAddedToCart)
    console.log(isAddedToCart)
  }

  // const counterForButton = () => {
  //   cart.map(cartItem => {
  ...
  const [countBuyElems, setCountBuyElems] = useState(0);
  ...
  return (
    <div>
      <div className="flex flex-col bg-white border-2 border-dashed border-black rounded-lg px-3 h-[625px] relative">
        ...
        {
          isAddedToCart ? (
            <div className="flex mx-auto border-2 border-dashed border-black hover:border-white p-2 mt-5 rounded-lg hover:bg-black hover:text-white duration-300"><Link to={'/cart'}>В корзину</Link></div>
          ) : 
          (
            <button
